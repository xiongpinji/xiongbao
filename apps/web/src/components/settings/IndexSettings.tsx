...
function KnowledgePanel() {
  ...
  async function write() {
    if (!id.trim() || !text.trim() || writing) return;
    setWriting(true);
    setError(null);
    try {
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
        ...
        <div className="space-y-3">
          ...
          <button className="primary-button" onClick={write} disabled={writing || !id.trim() || !text.trim()}>{writing ? "写入中…" : "写入"}</button>
      ...
      <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
        ...
        <div className="flex gap-2">
          <input className="field" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.nativeEvent.isComposing && search()} placeholder="输入检索问题" />
          <button className="primary-button" onClick={search} disabled={searching || !query.trim()}>{searching ? "检索中…" : "检索"}</button>
        </div>
        <div className="mt-4 space-y-2">
          {hits.length === 0 ? (
            <div className="rounded-lg border border-dashed border-neutral-700 p-4 text-sm text-neutral-500">暂无检索结果。</div>
          ) : (
            hits.map((hit) => (
              <div key={hit.id} className="rounded-lg border border-neutral-800 bg-neutral-950 p-3 text-sm text-neutral-300">
  ...
  return (
    <div className="rounded-lg border border-neutral-800 bg-neutral-900 p-5">
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_120px_auto]">
        <input className="field" placeholder="查询关键词，例如 vector database" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.nativeEvent.isComposing && search()} />
      ...
      <div className="mt-4 space-y-2">
        {results.length === 0 ? (
          <div className="rounded-lg border border-dashed border-neutral-700 p-4 text-sm text-neutral-500">暂无候选结果。</div>
        ) : (
          results.map((result) => (
            <div key={result.name + result.source} className="rounded-lg border border-neutral-800 bg-neutral-950 p-4">
