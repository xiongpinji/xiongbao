...
- [R4] 目标环境演练与发布证据归档 | Owner: Claude Code | 阻塞原因: 当前仅形成 lite/dev 级证明；full/staging 等价演练无法继续，因为缺少 `LANGFUSE_NEXTAUTH_SECRET`、`LANGFUSE_SALT`、`LANGFUSE_INIT_USER_PASSWORD`、full-mode 显式账号，且 compose 依赖端口 `5432/6379/6333/6334/3001/4000/8080/8081` 未监听；现有 R9 本地 UI smoke 也不能替代 R4 目标环境演练 | 需要谁决策: 环境/发布负责人提供可用 staging/full 演练环境与必填 secret，Claude Code 总调度复核恢复 | 恢复条件: 补齐上述 secret 与 full-mode 账号，拉起依赖服务，并提供至少一条 full-mode LLM 路径后，按 runbook 重跑 rehearsal 并归档证据 | 下次检查点: 上述环境条件满足后立即复检 | 证据: reports/delivery-report.md#r4-目标环境演练与发布证据归档
  live: 2026-07-07T00:44:47+08:00 | substate: WAITING_DEP | action: 等待环境/发布负责人根据 U2 恢复执行包补齐 full-mode secret、账号、LLM 路径与 compose 依赖服务 | risk: 当前只有 lite/dev 级证明，不能把本地健康检查或页面证据误判为 staging/full rehearshal 完成 | next: 按 `reports/delivery-report.md#u2-r4-full-mode-环境恢复执行包` 生成/重置 `.env.rehearsal`、完成端口预检与服务启动后再恢复 R4 | due: 2026-07-07T01:44:47+08:00 | detail: reports/delivery-report.md#u2-r4-full-mode-环境恢复执行包
- [R5] PR 审查包组装与 reviewer 入口清理 | Owner: Claude Code | 阻塞原因: R5 依赖 `R4` 目标环境演练与发布证据，以及 `R8` 对外口径终检；当前 `R4` 仍 BLOCKED，`R8` 尚未完成验收，缺少可签发的发布级输入 | 需要谁决策: Claude Code 总调度 / R4 环境负责人 / R8 reviewer | 恢复条件: `R4` 完成至少一次目标环境或 staging 等价演练并归档证据，且 `R8` 通过验收转 DONE 后，重新领取 R5 | 下次检查点: `R4` 与 `R8` 状态变化后立即复检 | 证据: reports/delivery-report.md#r4-目标环境演练与发布证据归档; reports/delivery-report.md#r8-对外口径一致性终检包
