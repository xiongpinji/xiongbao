...
- 当前结论是“本机单实例 Web/API 发布候选通过本地门禁”，不是远端已发布或客户生产已验收。当前提交尚未 push，未获得对应远端 CI、新 tag、GitHub Release 或生产部署证据。
- 现有 `v1.0.0` 标签不得移动或复用；后续发布必须选择与 API/Web/README 一致的新版本并重新完成远端发布签字。
