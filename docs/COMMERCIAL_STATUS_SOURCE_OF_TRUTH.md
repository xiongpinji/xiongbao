...
>
> **更新时间：** 2026-08-12（R3 独立验收后的 Web/API 发布候选口径）
>
> **适用对象：** owner、研发、QA、交付、下一位接手者。

---

## 0. 当前发布范围覆盖说明（2026-08-12）

- 当前候选只包含 **Web/API**；短剧业务链路和 Tauri 桌面端明确排除。
- 短剧由独立项目运行，稳定后按独立集成规格接入；仓库内相关源码不构成本阶段发布证据。
- P0/P1/P2、R1/R2 与 R3-A/R3-B 已在本地 `master` 完成；独立验收确认单一 50 样本真实 `qwen3:4b` 批次为 Chat 30/30、Scheduler 10/10、file_write 10/10，三类 P95 均低于冻结门槛，硬失败指标均为 0。
- 当前结论是“`1.1.0` 本机单实例 Web/API 发布候选通过本地门禁”，不是远端已发布或客户生产已验收。当前提交尚未 push，未获得对应远端 CI、`v1.1.0` tag、GitHub Release 或生产部署证据。
- 现有 `v1.0.0` 标签不得移动或复用；`1.1.0` 只有在候选分支 CI、master CI、新 tag/Release 和发布签字全部完成后才能写成正式发布。
...
## 0.3 2026-09-05 审计收口与 v1.2.0（当日全量实跑审计 → 缺口全闭环）

- **审计**：当日重建环境全量复验 v1.1.3 门禁（后端/前端/根级合同/鉴权/迁移全绿，口径零偏差）；
  发现两项流程缺口：①商用交付加固工作（PR #22，67 提交）在分支积压三周未合并，
  ②v1.1.x runtime 版本号停在 1.0.0（修复在未合并分支上）。
- **合并阻塞根因修复**：master 分支保护（ruleset + legacy branch protection）required
  check `backend` 与新 CI job 名不匹配，导致 PR #22 结构性不可合并（挂三周的直接原因）；
  已更新为实际 job 名（门槛增强为 2 个后端检查）。
- **PR #22 已合并**（squash → master 25d882c）：发布脊柱、目标环境门禁、版本对齐、
  安全扫描增强、桌面安装包 CI、性能修复全部进入主干；PR #21（哨兵锚点）随包并入后关闭。
- **v1.2.0 发布**：交付缺口修复（llm override 一键清除 API+CLI、环境一键重建脚本）+
  上下文压缩 V3-5（token 预算+摘要折叠，对标 Codex compaction）+ 竞品路线 V3-6/V3-7 立项。
- 审计报告：仓库根 `项目状态审计与竞品差距报告_20260905.md`；发布说明：
  `docs/RELEASE_NOTES_V1.2.0.md`。
- 边界不变：多机 HA、E2B L2、付费 provider、客户现场签字、桌面签名仍是独立外部条件。
...
- ✅ 自动恢复与自动归档（auto_recovery 引擎四类动作联动 + 阈值/节流/升级单测，auto_archive 归档联动含 manifest/recovery 日志/DB 证据导出）— 2026-08-04
- ✅ run / workflow 证据链自动生成（run.summary 标准化证据 + 归档导出 evidence-records.jsonl）— 2026-08-03（1dd4e02）
- ✅ 发布后观测结果自动汇总（post_deploy_summary.py 指标对比/HEALTHY·DEGRADED·ROLLBACK_RECOMMENDED 判定/CLI 端到端产物单测）— 2026-08-04

### P2（容量与性能实证）
- ✅ 正式压测基线（两轮：基线 + soak/Postgres/多 worker）— 2026-08-03
- ✅ 多实例一致性与扩容实证（共享状态限流/登录锁定/调度锁 8-3 实测；共享 PG 数据一致性 + JWT 无状态会话同机两实例实测 2026-08-04；多 worker 扩容 6.4× 量化 8-3 压测）
- ✅ 队列 / 缓存 / LLM 路径瓶颈治理（三瓶颈修复实测；skills/login 路径）— 2026-08-03

### P3（商业复制增强）
- ✅ 标准交付模板与角色包（delivery-templates/：试点包/升级包/恢复包/角色包）— 2026-08-03（1599ce7）
- ✅ 标准试点包 / 升级包 / 恢复包（同上）— 2026-08-03
- 行业 / 场景交付变体包（指南已立；✅ 研发效能变体包 v1 + 企业内部助手变体包 v1 2026-08-04；短剧变体 ⏸️ 已暂停（owner 决策 2026-08-04），待整体移植后重启）
...
截至 2026-08-12，当前本地 `master` 已完成冻结 Web/API 范围内的 P0/P1/P2、R1/R2 和 R3-A/R3-B。R3 独立验收复算了不可变 50 样本批次，复读数据库、checkpoint、Scheduler、开发任务与 patch 清理证据，并在当前 Python 3.13 环境重新通过 R3/R2 合同、完整 Web/API 后端发布范围和精确静态质量门。

> 可以表述为：`xagent` 当前具备本机单实例、受控私有部署的 Web/API 发布候选能力。不得表述为“当前提交已正式发布”或“已在全部企业场景与 Codex/Hermes 完全等价”；远端 push/CI、新版本 tag/Release、生产部署、多机 HA、E2B、付费 provider 和客户现场仍是独立门禁。
