...
>
> **更新时间：** 2026-08-07（当前 Web/API 竞品差距硬化范围覆盖此前宽口径结论）
>
> **适用对象：** owner、研发、QA、交付、下一位接手者。

---

## 0. 当前发布范围覆盖说明（2026-08-07）

- 当前正在验收的候选只包含 **Web/API**；短剧业务链路和 Tauri 桌面端明确排除。
- 短剧由独立项目运行，稳定后按独立集成规格接入；仓库内相关源码不构成本阶段发布证据。
- 当前分支属于 v1.0.0 之后的未发布硬化工作。现有 `v1.0.0` 标签不得移动或复用，实际发布必须选择与 API/Web/README 一致的新标签。
- 在 Web/API P0 发布基础、P1 开发任务闭环、P2 持久调度/技能/会话恢复和 R1 发布级审计全部通过前，不得把本轮竞品差距补齐表述为“已达到发布标准”。
- 下文 G1/G2/G3 结论是此前验收定义下的历史结论，不自动覆盖本轮更严格的 Web/API 发布门禁与 Codex/Hermes 对标目标。
...
G1/G2/G3 已全部完成，项目按当前定义达到商用交付标准，处于 Roadmap v2 增强期。截至 2026-08-03：P0 平台化（secretRef + Helm + 配置治理收口 2026-08-04）与 P2 容量实证（三瓶颈修复 + soak/Postgres/多 worker 基线）已落地并实测；新候选（c175201 后 221 提交，PR #7 HEAD 2aae3a2）已获 owner 重新签发（见 R5 包 §0.4）；**PR #7 已于 2026-08-05 以 merge commit `c2260cd` 合入 master（签发延伸见 R5 包 §0.5，合并前 CI run 30964570430 全绿）**。剩余边界为——多机 HA 未演练（共享状态与数据一致性已同机两实例实测）、L2 E2B 云沙箱未实测（无 key）、非当前机器的目标环境演练未做。
